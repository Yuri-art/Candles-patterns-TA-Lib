...
def find_volume_levels(data, bins=30):
    """
    Определяем уровни поддержки и сопротивления по объемам (Volume Profile).
    """
    volume_profile, bin_edges = np.histogram(data['close'], bins=bins, weights=data['volume'])
    levels = bin_edges[np.argsort(volume_profile)[-10:]]  # Берем 10 уровней с наибольшим объемом

    return sorted(levels)


#  Находим уровни по объемам
volume_levels = find_volume_levels(crypto)
print("Уровни по объему:", volume_levels)
...
#  Фильтруем только те паттерны, которые находятся около объёмных уровней
patterns_near_volume = crypto[(crypto['candlestick_pattern'] != '') & crypto['near_volume_level']]

#  Создание папки для графиков
output_folder = 'output/charts-volume-levels'
os.makedirs(output_folder, exist_ok=True)

#  Визуализация графиков с объемными уровнями и подтвержденными паттернами
for candle in candle_names:
    pattern_data = patterns_near_volume[patterns_near_volume['candlestick_pattern'].str.contains(candle)]
